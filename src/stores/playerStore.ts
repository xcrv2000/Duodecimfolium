...
export const usePlayerStore = create<PlayerStore>((set) => ({
  ...
  craftCard: (cardId, cost) => set((state) => {
    ...
    // Use existing addCard logic to handle potential overflow (though crafting implies count < 3 usually)
    // But we need to deduct dust FIRST, then add card.
    // However, addCard uses `set` which merges state. We can't easily compose them inside `set`.
    // We'll duplicate the add logic or call it? 
    // Zustand `set` receives current state.
